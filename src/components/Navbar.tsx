...
export default function Navbar({ currentPage, setCurrentPage }: NavbarProps) {
  ...
  return (
    ...
    >
      <button
        onClick={goHome}
        className="font-syne font-bold text-sm tracking-widest uppercase transition-colors duration-200 cursor-pointer"
      ...
      {/* Desktop links - only show when on home page */}
      {currentPage === 'home' && (
        <div className="hidden min-[990px]:flex items-center gap-1">
          {links.map((l) => {
      ...
      {/* Desktop links - service page back button */}
      {currentPage !== 'home' && (
        <button
          onClick={goHome}
          className="hidden min-[990px]:flex items-center gap-2 font-syne text-xs tracking-widest uppercase px-3 py-2 transition-colors duration-200"
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="min-[990px]:hidden flex flex-col gap-1.5 p-2"
        ...
        >
          {currentPage === 'home' ? 'Hablemos' : 'Inicio'}
        </button>
      </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className="absolute top-full left-0 right-0 min-[990px]:hidden"
          ...
          >
            <div className="flex flex-col px-6 py-6 gap-2">
              {currentPage === 'home' ? (
                ...
                <motion.button
                  onClick={goHome}
                  className="relative font-syne text-sm tracking-widest uppercase py-3 pl-4 transition-colors duration-200 text-left w-full"
                  style={{ color: 'var(--accent)' }}
                  initial={{ opacity: 0, x: -12 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.2, ease: [0.25, 0.1, 0.25, 1] }}
